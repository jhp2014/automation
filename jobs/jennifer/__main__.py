...
#   운영 머신 검증 결과 카운트는 새로고침(HTTP)으로 채워져 WS 차단 후에도 fatal/
#   total 이 정상 추출됨을 확인 -> True 로 둔다. 만약 추후 대시보드 변경으로 값이
#   안 들어오면 div.fatal visible 대기에서 타임아웃 -> 러너 안전망 알림으로
#   잡히므로 "조용한 정상 오인"이 아니라 "실패 알림"으로 빠진다(False 로 되돌리면 됨).
BLOCK_POPUP_WS = True
